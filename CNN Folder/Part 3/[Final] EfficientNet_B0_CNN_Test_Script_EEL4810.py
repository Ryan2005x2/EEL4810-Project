# This is the final, improved program that takes a satellite image and predicts if it shows wildfire smoke by running it
# through the previously trained [Final] EfficientNet_B0_CNN_Train_and_Val_Script_EEL4810.py.
...
def preprocess_image(raw_image):
    ...
    transform = transforms.Compose([
        ...
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), # Normalize with ImageNet mean and stdev
    ])

    image = transform(raw_image)
    
    print("Preprocessed image shape:", image.shape)
    print("Preprocessed image min/max:", image.min().item(), image.max().item())
...
def main():
    ...
    image_path = r"C:\Users\orang\Downloads\maxresdefault-952232919.jpg"
